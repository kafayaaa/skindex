  ...
  const generateCalendarDays = (): CalendarDay[] => {
    ...
    for (let i = firstDayOfWeek - 1; i >= 0; i--) {
      ...
      days.push({
        ...
        skinRating: Math.random() > 0.5 ? Math.floor(Math.random() * 5) + 1 : undefined
      })
    }
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push({
        ...
        skinRating: Math.random() > 0.4 ? Math.floor(Math.random() * 5) + 1 : undefined
      })
    }
    ...
    <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-6">
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-zinc-600 dark:text-zinc-400">Hari ini</span>
        </div>
        <div className="flex items-center gap-2">
          <CheckCircle className="w-4 h-4 text-green-500" />
          <span className="text-sm text-zinc-600 dark:text-zinc-400">Log skincare</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-green-500"></div>
          <span className="text-sm text-zinc-600 dark:text-zinc-400">Kulit baik (4-5)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
          <span className="text-sm text-zinc-600 dark:text-zinc-400">Kulit sedang (3)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-red-500"></div>
          <span className="text-sm text-zinc-600 dark:text-zinc-400">Kulit kurang (1-2)</span>
        </div>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-1">
                    ...
                    {day.skinRating && (
                      <div 
                        className={`w-2 h-2 rounded-full ${getRatingColor(day.skinRating)}`}
          ...
          <div className="bg-zinc-50 dark:bg-zinc-900/50 rounded-xl p-5 h-full">
            ...
            {selectedDayDetails ? (
              <div className="space-y-4">
                {/* Skin Status */}
                <div>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-2">Status Kulit</p>
                  {selectedDayDetails.skinRating ? (
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${getRatingColor(selectedDayDetails.skinRating)} text-white`}>
                        {selectedDayDetails.skinRating}
                      </div>
                      <div>
                        <p className="font-medium">
                          {selectedDayDetails.skinRating <= 2 ? 'Perlu Perhatian' :
                           selectedDayDetails.skinRating <= 3 ? 'Cukup Baik' : 'Sangat Baik'}
                        </p>
                        <p className="text-sm text-zinc-500 dark:text-zinc-400">
                    ...
                    <p className="text-zinc-500 dark:text-zinc-400 italic">Belum ada data</p>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-zinc-700 dark:text-zinc-300">Log Skincare</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-zinc-700 dark:text-zinc-300">Analisis Foto</span>
                ...
                {/* Notes Section */}
                <div>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-2">Catatan</p>
                  {selectedDayDetails.notes ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      ...
                      <p className="text-zinc-500 dark:text-zinc-400">Belum ada catatan</p>
      ...
      <div className="mt-8 pt-6 border-t border-zinc-200 dark:border-zinc-700">
        <h4 className="font-medium text-zinc-900 dark:text-zinc-100 mb-4">Ringkasan Bulan Ini</h4>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-4 rounded-lg bg-cyan-50 dark:bg-cyan-900/20">
            ...
            <p className="text-sm text-zinc-600 dark:text-zinc-400">Hari dengan log</p>
          </div>
          <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/20">
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">
              {calendarDays.filter(d => d.hasAnalysis && d.isCurrentMonth).length}
            </p>
            <p className="text-sm text-zinc-600 dark:text-zinc-400">Analisis foto</p>
          </div>
          <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20">
            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">
              {Math.round(
                ...
                calendarDays.filter(d => d.skinRating && d.isCurrentMonth).length || 0
              ).toFixed(1)}
            </p>
            <p className="text-sm text-zinc-600 dark:text-zinc-400">Rata-rata rating</p>
          </div>
          <div className="p-4 rounded-lg bg-purple-50 dark:bg-purple-900/20">
            <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">
              {calendarDays.filter(d => d.skinRating && d.skinRating >= 4 && d.isCurrentMonth).length}
            </p>
            <p className="text-sm text-zinc-600 dark:text-zinc-400">Hari kulit baik</p>
          </div>
        </div>
      </div>
    </div>
